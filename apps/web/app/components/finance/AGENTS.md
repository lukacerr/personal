...
- **La lista ordena con `sortPayments`: primero lo puntual, más nuevo arriba, y las suscripciones al final** bajo un divisor. El `paidAt` de una suscripción es cuándo arrancó, no cuándo se pagó, así que en un período que solo solapa la columna Date estaría informando algo que no pasó entonces — por eso va vacía para esas filas. El orden es por `paidAt` y no por monto, que necesitaría una cotización para comparar dos monedas y se reacomodaría cada vez que se mueve el dólar.

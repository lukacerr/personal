...
- `--env-file` de Bun no pisa una variable que ya está en `process.env`, y `apps/api/src/env.ts` usa `emptyStringAsUndefined`. Un ambiente que exporte una credencial vacía hace fallar la validación nombrando justo una variable que sí está definida en `.env.test`, y los demás archivos de test mueren después con errores TDZ que no dicen nada. Los agentes corren dentro de ese ambiente: Claude Code exporta `ANTHROPIC_API_KEY` vacía. Si la suite de API muere en `env.ts`, comprobá el ambiente antes que el archivo; `env -u VARIABLE bun test --env-file .env.test` confirma la causa en un solo comando.

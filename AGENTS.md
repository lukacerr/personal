...
- Un atajo global del shell que también es atajo de edición de texto —`Ctrl/Cmd+B` togglea el sidebar y es bold en cualquier editor— se descarta cuando el `target` del evento es editable (`isContentEditable`, `input`, `textarea`), no cuando hay una selección: con el caret colapsado el editor igual aplica el formato a lo que se escriba después, así que el guard por selección deja el atajo doble justo en el caso común. El guard vive en `sidebar-size.ts` porque `components/ui/sidebar.tsx` es vendored y Biome no lo revisa.

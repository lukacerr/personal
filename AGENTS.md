...
| --- | --- | --- |
| `apps/api/AGENTS.md` | Agent, Calendar, Credentials, Finance, Notes y Storage server-side: chat SSE con AI SDK y registries de modelos/tools, fechas locales y LWW de eventos, sobres cifrados y verificación de escritura, cotización congelada y best-effort, modelo de versiones, deltas, multipart, reconcile, routers públicos | `apps/api/src/agent*.ts`, `credentials*.ts`, `events.ts`, `payments.ts`, `dolar.ts`, `notes.ts`, `note-versions.ts`, `public-notes.ts`, `files*.ts`, `public-files.ts`, `schema/agent-*.ts`, `schema/credential.ts`, `schema/event*.ts`, `schema/payment.ts`, `schema/note*.ts`, `schema/file.ts` |
| `apps/web/app/components/agent/AGENTS.md` | Agent en la web: chat streaming con useChat, transport autenticado fuera de Eden, selección model/reasoning/tools por turno, rail de threads, elements vendoreados de AI Elements, document scroll con composer sticky | `apps/web/app/components/agent/**`, `app/components/agent-bootstrap.tsx`, `app/lib/agent*.ts`, `app/routes/_app.agent.tsx` |
| `apps/web/app/components/calendar/AGENTS.md` | Calendar en la web: una sola entidad evento, fechas locales string, expansión de series en cliente, cola coalescida con reloj LWW, ventana de 14+ días, quick-add, Schedule anclado | `apps/web/app/components/calendar/**`, `app/lib/calendar*.ts`, `app/routes/_app.calendar.tsx` |
...
| `apps/web/app/components/finance/AGENTS.md` | Finance en la web: rango elegido a mano y recordado, las dos puntas del spread, ventana de suscripciones, totales por moneda, desglose por tag | `apps/web/app/components/finance/**`, `app/lib/finance*.ts`, `app/routes/_app.finance.tsx` |
...
- Que un valor esté parametrizado no lo vuelve inerte. Cuando input de usuario alimenta un operador que interpreta su contenido (`LIKE`/`ILIKE`, `SIMILAR TO`, regex, `to_tsquery`), escapa los metacaracteres del operador y declara el `ESCAPE` correspondiente. Un `_` o `%` sin escapar en un prefijo convierte una operación puntual en una masiva. Para carpetas por prefijo ya existe `@api/folder-paths` (`likeDescendantsOf`/`inFolder`, parametrizado por columna); úsalo en vez de reescribir el escape.
- Acota siempre los rangos numéricos que provienen del cliente, no solo su tipo. Los timestamps se validan con `@api/validation` y la cota depende de qué es el valor: un **reloj de edición** (LWW, historial) usa `timestampMs`, acotado a `Date.now()` + 48 h de skew evaluado por request — sin esa cota, un reloj del futuro lejano gana toda resolución last-write-wins para siempre —; una **fecha elegida por el usuario** (fin de suscripción, límites de un período) usa `dateTimestampMs`, acotada solo al máximo de `Date`, porque su futuro es legítimo. No copies estos validadores en los routers.
...
- **Un store cuyo contenido es un índice del server se arma con `createIndexCore` (`app/lib/index-store.ts`)**, que es dueño del request coalescido, la guarda de sesión chequeada a la entrada y después de cada await, las transiciones de `status`/`error`, los mutadores que invalidan el tag y el outcome que devuelve `load`. Finance, Storage y Credentials lo usan; Agent comparte las piezas pero no la forma, porque páginas y filtro de título no son un índice de una sola lista. Existe porque los cuatro tenían una copia de esa coreografía y **ya habían divergido**: uno liberaba su slot sin chequear identidad y otro había perdido la guarda de sesión entera, que es lo que dejaba a un pull repoblar el store después del sign-out. Cada store le pasa un `patch` concreto en vez de que el core escriba con una key computada: así el core no necesita un solo cast y cada store le sigue llamando a su colección como sus consumidores.
- **La mitad cliente del ETag vive una sola vez en `app/lib/http-conditional.ts`** (`conditionalGet`), espejo de `apps/api/src/http-cache.ts`: manda el `if-none-match` a través de `fetch` —no de `headers`, que Eden tipa con el único header de su propio contrato—, traduce el `304` a `'unchanged'` y saca el tag nuevo de la respuesta. El helper es dueño del protocolo; cada caller sigue siendo dueño de su contrato, valida su propia forma y tira su propio `*ApiError`. Un caller que solo revalida algunos de sus requests lo expresa **no pasando** el tag, como `listThreads` con cursor o query. `notes-sync.ts` queda afuera a propósito: adopta el tag recién después de un reconcile async con guarda de sesión a los dos lados.
...
- **El header del shell (breadcrumb, badges) queda fijado al viewport** (`sticky top-0 z-30` dentro del inset, que es lo que lo mantiene alineado con un sidebar redimensionable sin coordinar offsets). Toda toolbar de pantalla que se pinne va **debajo suyo con `top-16`**, nunca `top-0`: con el header fijado, `top-0` la deslizaría por detrás.
...
- Un atajo que también es atajo de edición de texto —`Ctrl/Cmd+B` togglea el sidebar y es bold en cualquier editor; una letra suelta es texto en cualquier campo— se descarta cuando el `target` del evento es editable (`isContentEditable`, `input`, `textarea`), no cuando hay una selección: con el caret colapsado el editor igual aplica el formato a lo que se escriba después, así que el guard por selección deja el atajo doble justo en el caso común. Ese guard es `isEditableTarget` en `app/lib/keyboard.ts`, junto al tipo `ShortcutEvent` que hace testeable un predicado con un objeto literal e `isBareLetterShortcut`, que concentra las condiciones que toda letra pelada comparte (sin modificadores, sin repeat, target no editable); los predicados concretos viven con su dueño y delegan en ese helper (`isAppSidebarShortcut` en `sidebar-size.ts`, porque `components/ui/sidebar.tsx` es vendored y Biome no lo revisa; `isAddPaymentShortcut` en `finance.ts`; `isAddCredentialShortcut`, `isToggleRevealShortcut` e `isCopyCredentialShortcut` en `credentials.ts`). No redeclares la condición: cada copia es una que se olvida de un caso.
- **Un atajo no se queda con una tecla que el browser ya usa para algo que alguien hace.** `Ctrl/Cmd+A` es seleccionar todo y no se toma; la acción va en la letra suelta, que fuera de un campo de texto no cuesta nada. Un atajo de pantalla además se saltea mientras hay un diálogo abierto: dentro de uno el foco suele estar en un botón, no en un campo, así que el guard de editable solo no alcanza y la tecla descartaría una edición a medias.
...
- `/` no es una pantalla: redirige a `APP_DEFAULT_PATH` (`app-navigation.ts`), y solo llega ahí quien no tenía nada guardado para restaurar. La constante vive junto a los paths con los que tiene que coincidir y un test verifica que sigue siendo una entrada real de `appNavigation`; si no, cada arranque limpio iría a una ruta que no resuelve.
- La restauración **no** valida que la ruta guardada siga existiendo, y es deliberado: al sacar un system, un dispositivo que lo tuviera abierto último abre en 404 una vez y se corrige solo al navegar. Un fallback que reescribiera el destino en silencio esconde una ruta muerta en vez de mostrarla, y el shell tendría que conocer el catálogo de pantallas para decidirlo.
...
- La paleta agrupa las soluciones **en el orden del sidebar**, no en el del array `appSystems`: `systemsInSidebarOrder()` ordena por la posición en `appNavigation` usando `/<key>`, así que la key de un system tiene que nombrar su ruta. Ordenar en el punto de uso hace que sumar un system siga siendo un append de una línea, sin una segunda lista que mantener sincronizada; un test lo fija.
- El shell no conoce ninguna solución concreta. Cada system aporta sus comandos, su cola de breadcrumb y su resumen de sidebar registrándose en `appSystems` (`apps/web/app/lib/app-systems.ts`) con un módulo propio tipo `notes-system.ts`; `app-navigation.ts`, `app-breadcrumb.tsx`, `app-command-palette.tsx` y `app-sidebar.tsx` se mantienen genéricos. Sumar una solución es agregar una entrada al registro, nunca editar el shell.
...
- **El resumen de sidebar es `AppSystem.loadSummary`, y su forma es una sola para los cuatro systems que lo usan**: filas `{ label, detail?, to? }`, donde `label: 'Left over'` + `detail: '$ 123.456'` es la misma fila que `label: 'Dentista'` + `detail: '火 15:00'`. Una forma por system obligaría al shell a renderizar Finance distinto de Calendar, que es justo lo que no puede saber; lo único que decide el shell es si una fila lleva a algún lado (`SidebarMenuButton` con `NavLink`) o solo se lee (`<dl>`). No toma argumentos: se renderiza al lado de la navegación, donde no hay ruta ni selección, así que un system que quiera decir algo ahí tiene que poder decirlo desde cualquier pantalla. **Un system sin filas no renderiza grupo** — el estado normal de Notes es no tener drafts, y cuatro cajas vacías permanentes son peores que ninguna —, y esa condición vive en `loadSystemSummaries`, no en el componente. Un `to` tiene que ser una url que la pantalla destino realmente consuma: Calendar deja sus filas sin link por la misma razón por la que su breadcrumb está vacío, porque su url no lleva selección y un `?date=` inventado sería un link muerto. Los grupos del resumen siguen el orden del sidebar salvo que un system declare `summaryRank` (default 0, sort estable): solo mueve su grupo de resumen, nunca su entrada de navegación ni el orden de la palette — Agent lo usa para leer sus conversaciones debajo de Calendar y Finance.
- **Un resumen informa, no levanta la voz: todas las filas se renderizan con el mismo peso.** No hay campo `alert` ni color por fila. Lo que tiene que decir que algo anda mal lo dice con palabras — Notes escribe `Failed`, un Left over negativo ya viene con signo —, porque un color solo no es un mensaje que alguien pueda leer en voz alta, y cuatro cajas compitiendo por atención dejan de ser un resumen.
- **El dato de una fila lo escribe el system y puede ser largo; el `label` trunca y el `detail` nunca se pierde.** Una fila del resumen es un grid item, y `truncate` en el label no alcanza: un hijo `flex-1` hace que el min-content de su fila sea el ancho completo del texto, y un grid item no baja de su min-content mientras su `min-width` sea `auto`. Un solo título largo ensanchaba así la columna compartida, y como `SidebarContent` es `overflow-auto` con `no-scrollbar`, el día/hora de **todas** las filas del grupo quedaba scrolleado fuera de vista, incluidas las de títulos cortos. Por eso la fila lleva su propio `min-w-0`. Las filas con `to` se salvan solas porque son botones `w-full`, que nunca se dimensionan por su contenido.
- **La UI va en inglés.** Los textos que escribimos nosotros, no los datos: el título de un evento o de una nota es de Luka y queda como está. Calendar es la excepción deliberada y habla su propia notación (`月火水木金土日`, `今日`, `試験`), que es la que la nota original usaba y la que hace que heading, badge y sidebar se lean como una sola cosa.
- **El resumen no se resuelve mientras el sidebar está cerrado.** Colapsado sigue en el DOM (por eso lleva `inert`), así que sin esa compuerta los cuatro systems seguirían contestando preguntas cuya respuesta nadie puede ver. La compuerta es `isMobile ? openMobile : open` y entra como dependencia de la `useLiveQuery`, no como un `return` temprano que se saltearía el hook.
- **Volver a una máquina que quedó abierta es del shell, no de cada system.** `useSystemRefresh` (`app/lib/system-refresh.ts`, montado en `_app`) pullea el system de la ruta activa; un system aporta ese pull con `AppSystem.refresh(search)` y nada más. Recibe el search porque lo que está en pantalla puede ser parte de lo que hay que traer: Notes le pasa la nota abierta a `refreshNotes`, que ya se niega a pisar una nota con draft u operaciones en cola. Antes esto vivía duplicado en `notes-bootstrap.tsx` y en un efecto de la ruta de Calendar, y Finance y Credentials directamente no lo tenían.
- **Un system que hay que refrescar fuera de su pantalla lo declara con `refreshEverywhere`, no monta un componente.** Solo por dos razones que se sostienen: su índice se lee desde cualquier lugar (la palette consulta Notes y Agent en toda la app; el resumen del sidebar lee los totales de Finance) o guarda una cola que tiene que salir en cuanto haya conexión (el outbox de Calendar, que si no esperaba la próxima visita a `/calendar`). Off-route el pull va **sin search** — no hay selección en una pantalla que no es la suya — y en su propia pantalla el system es un solo target, porque el de la ruta va primero y gana el dedupe; ese dedupe es load-bearing en la reconexión, que saltea la compuerta de antigüedad y si no dispararía dos requests para un system. Está apagado por defecto: un system cuyos datos nadie lee desde otra pantalla estaría gastando requests que nadie mira, justo lo que la regla de no-polling evita. Esto reemplazó al `AppSystem.Bootstrap`, que era la puerta por la que cada system se hacía su propia versión de pull inicial, listener de `online`, frescura y guarda de sesión — y `AgentBootstrap` ya había divergido de `NotesBootstrap`, del que era copia.
- **El resultado de un pull lo devuelve la llamada, no se lee del `status` después.** `AppSystem.refresh` resuelve `boolean` y `load` de un index store devuelve su `IndexLoadOutcome`. `status` es estado compartido: con dos llamadas entrelazadas —un refresh y una búsqueda del rail— leerlo después del await reporta el resultado de la otra, y así el coordinador acreditaba como fresco un pull que había fallado y aplicaba backoff a uno que había funcionado.
- **No hay polling, y es el punto: una app que nadie toca no emite un solo request** y deja dormir el container serverless. El disparador son señales de vida — `pointerdown`, `pointermove`, `keydown`, `wheel`, `touchstart` — más `focus`, `online` y `visibilitychange`. Los tres últimos **no alcanzan solos**, que es el caso que motivó todo esto: la ventana puede quedar visible y enfocada toda la tarde mientras no hay nadie, y ahí mover el mouse es la única evidencia de que alguien volvió. **Sumar un system a `refreshEverywhere` no toca esta invariante** — cambia qué cubre un pull, no cuándo ocurre —, y hay un test que la fija: "never pulls on time alone" avanza ocho horas de reloj sin emitir una señal y exige cero pulls, así que falla el día que alguien alcance un `setInterval`. **`useApiHealth` va más lejos y no escucha nada: pregunta una vez al abrir la app y después solo cuando alguien toca el badge** (`onRecheck`). Escuchaba `online` y `visibilitychange`, que no es un poll pero se comporta como uno en uso normal — cada vuelta a la pestaña gastaba un request, así que una ventana que alguien saltea toda la tarde no dejaba dormir nunca al container. No se pierde nada: la respuesta tampoco era confiable entre chequeos, porque la API puede caerse un segundo después de cualquiera. Por eso el badge dice **cuándo** miró por última vez y es un `<button>` de verdad, con nombre accesible que incluye la acción. Lo que sí cambia solo es la conectividad, y eso es `usePwaAvailability`, que lee `navigator.onLine` y no cuesta un request.
- El corte es la **antigüedad del dato, no un intervalo**: `SYSTEM_REFRESH_STALE_MS` (180 s) por system, no global, porque llegar a una pantalla es el único momento en que su frescura se puede juzgar — Finance, Storage y Credentials se saltean su propia carga cuando el store ya está `ready`. Un pull fallido entra en un backoff acotado (5–60 s): reintenta antes que el dato fresco normal sin convertir cada `pointermove` en un request. Como los pulls coalescen llamadas concurrentes, el request que dispara llegar a una pantalla es el mismo que la ruta ya estaba haciendo al montar; no hace falta que el coordinador adivine si la ruta pullea sola.
- Todo refresh iniciado por el shell lleva una guarda de generación de sesión. Sign-out invalida la generación **antes** de limpiar datos locales y bloquea trabajo nuevo hasta la próxima autenticación; Calendar y Notes comprueban esa guarda antes de cada commit posterior a red para que una respuesta en vuelo no repueble Dexie, y los stores en memoria de Finance, Storage y Credentials hacen lo mismo antes de cada `set` posterior a red. Esos stores además exponen `reset` y se registran en `clearLocalData`: cerrar sesión borra índices, tags, errores y requests en vuelo, no solo las bases persistidas.
- **Un refresh de fondo no puede vaciar una pantalla que ya tiene contenido.** Los tres stores ponen `status: 'loading'` durante todo el viaje, así que cualquier `loading` derivado va condicionado a que la lista esté vacía (`files.length === 0` y equivalentes). Sin eso, cada refresh reemplaza por un spinner una lista sobre la que el usuario todavía puede actuar. Un fallo de fondo, en cambio, se calla: el system lo muestra donde vive su propio estado y la próxima señal reintenta; el que avisa es el botón manual, que sí fue una acción explícita.
...
- **Los dos clientes Eden llevan `parseDate: false` y debe quedarse así.** Por defecto, el reviver de Eden corre sobre cada string de cada respuesta y convierte en `Date` cualquier cosa con forma de fecha — el `YYYY-MM-DD` local de Calendar, o una nota titulada `2026-01-01` — aunque el contrato declare string. Costó un crash en runtime (`date.split is not a function`) que el typecheck no puede ver. Los timestamps de este repo viajan como epoch ms numéricos, así que nada legítimo depende de esa conversión.
...
- `--env-file` de Bun no pisa una variable que ya está en `process.env`, y `apps/api/src/env.ts` usa `emptyStringAsUndefined`. Un ambiente que exporte vacía una variable del schema hace fallar la validación nombrando justo una variable que sí está definida en `.env.test`, y los demás archivos de test mueren después con errores TDZ que no dicen nada. Por eso el schema declara únicamente variables que el código consume: cada key de más es una colisión posible con el ambiente de quien corre la suite (mientras el schema exigía API keys que nadie usaba, el script `test` necesitó un workaround `env -u`). Si la suite muere en `env.ts` nombrando una variable presente en `.env.test`, comprobá el ambiente antes que el archivo: `env -u VARIABLE bun test --env-file .env.test` confirma la causa en un solo comando.
...
- La base de desarrollo es completamente local y no requiere una cuenta de Neon. La API conserva `@neondatabase/serverless`: en Compose, `NEON_FETCH_ENDPOINT=http://db-proxy:4444/sql` dirige `neon()` al shim; fuera de Docker usa `http://localhost:4444/sql`; en producción se omite para usar Neon Cloud.
- Redis Insight expone la UI de cache en `localhost:5540`. PostgreSQL, el shim HTTP de Neon, Redis TCP, REST de cache y MinIO también se publican solo en loopback para debugging.
...
- El pull **vacía las tablas de `SECRET_TABLES`** (hoy `credential`) al final del restore. Sus filas están cifradas con el `LUKA_SECRET` de producción, que local no tiene, así que llegarían indescifrables y la pantalla sería una pared de filas que nadie puede abrir. Un system nuevo que cifre filas con `LUKA_SECRET` se agrega a esa lista.
- El borrado va como un `--command` **después** del `--file`, dentro de la misma transacción. `psql` procesa `--command` y `--file` en el orden en que aparecen, así que ese orden es el comportamiento, y adelantarlo rompe de dos maneras: después del `CREATE SCHEMA` pero antes del dump falla con la tabla inexistente y tira todo el restore, y antes del `DROP` borra las filas que estaba por reemplazar y después llegan las de producción intactas — sin error, sin salida, y con todos los secretos en la base local. Por eso `restoreArgs` es una función exportada con test propio en vez de argumentos inline.
- **El borrado va calificado con el esquema (`DELETE FROM public.credential`).** `pg_dump` cierra el archivo con `set_config('search_path', '', false)` y crea todo calificado, y ese setting sobrevive al `--file`: un statement sin esquema después falla con `relation "credential" does not exist` aunque `public.credential` acabe de crearse. Costó un pull fallido descubrirlo; un test lo fija ahora. Reproducirlo en local exige emitir el mismo `set_config`, no basta con un archivo SQL cualquiera.
- Las notas que referenciaban una credencial no se tocan: su bloque guarda el id y reporta una referencia que ya no está, que es exactamente lo que la copia local debe decir.

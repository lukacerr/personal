...
- Notes usa Dexie `personal-notes:v1` (schema v1) para cache y outbox. `dirty` representa exclusivamente un draft local real, no el desmontaje del editor; `draftUpdatedAt` decide LWW frente a `updatedAt` remoto. Una nota limpia seleccionada se refresca desde la API y el snapshot aceptado debe aplicarse al BlockNote ya montado sin disparar otro draft. `title`/`path` son metadata: se sincronizan inmediatamente mediante PATCH/outbox para notas ya creadas, mientras el contenido solo crea versiones con Save. `serverUpdatedAt` representa exclusivamente una versión de contenido confirmada por servidor. `syncFailure` marca una nota cuya operación fue rechazada de forma terminal. Las preferencias de fuente/márgenes son locales por dispositivo bajo `personal-notes-view:v1`; la última nota abierta se recuerda bajo `personal-notes-selection:v1`.
...
- La UI de Notes vive en `apps/web/app/components/notes` (`notes-tree`, `note-document`, `note-history`, `note-math`, `note-preferences`); `apps/web/app/routes/_app.notes.tsx` solo orquesta estado, diálogos y layout. Los helpers puros del editor están en `apps/web/app/lib/notes-editor.ts` y las operaciones sobre la base local en `notes-db.ts`. No devuelvas lógica de datos ni componentes al archivo de ruta.

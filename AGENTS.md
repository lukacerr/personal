...
| --- | --- | --- |
| `apps/api/AGENTS.md` | Credentials, Finance, Notes y Storage server-side: sobres cifrados y verificación de escritura, cotización congelada y best-effort, modelo de versiones, deltas, multipart, reconcile, routers públicos | `apps/api/src/credentials*.ts`, `payments.ts`, `dolar.ts`, `notes.ts`, `note-versions.ts`, `public-notes.ts`, `files*.ts`, `public-files.ts`, `schema/credential.ts`, `schema/payment.ts`, `schema/note*.ts`, `schema/file.ts` |
...
- Un atajo que también es atajo de edición de texto —`Ctrl/Cmd+B` togglea el sidebar y es bold en cualquier editor; una letra suelta es texto en cualquier campo— se descarta cuando el `target` del evento es editable (`isContentEditable`, `input`, `textarea`), no cuando hay una selección: con el caret colapsado el editor igual aplica el formato a lo que se escriba después, así que el guard por selección deja el atajo doble justo en el caso común. Ese guard es `isEditableTarget` en `app/lib/keyboard.ts`, junto al tipo `ShortcutEvent` que hace testeable un predicado con un objeto literal; los predicados concretos viven con su dueño (`isAppSidebarShortcut` en `sidebar-size.ts`, porque `components/ui/sidebar.tsx` es vendored y Biome no lo revisa; `isAddPaymentShortcut` en `finance.ts`). No redeclares la condición: cada copia es una que se olvida de un caso.

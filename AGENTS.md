...
| --- | --- | --- |
| `apps/api/AGENTS.md` | Calendar, Credentials, Finance, Notes y Storage server-side: fechas locales y LWW de eventos, sobres cifrados y verificación de escritura, cotización congelada y best-effort, modelo de versiones, deltas, multipart, reconcile, routers públicos | `apps/api/src/credentials*.ts`, `events.ts`, `payments.ts`, `dolar.ts`, `notes.ts`, `note-versions.ts`, `public-notes.ts`, `files*.ts`, `public-files.ts`, `schema/credential.ts`, `schema/event*.ts`, `schema/payment.ts`, `schema/note*.ts`, `schema/file.ts` |

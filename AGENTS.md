...
- La UI de Notes vive en `apps/web/app/components/notes` (`notes-tree`, `note-document`, `note-history`, `note-preferences`); `apps/web/app/routes/_app.notes.tsx` solo orquesta estado, diálogos y layout. Los helpers puros del editor están en `apps/web/app/lib/notes-editor.ts` y las operaciones sobre la base local en `notes-db.ts`. No devuelvas lógica de datos ni componentes al archivo de ruta.
